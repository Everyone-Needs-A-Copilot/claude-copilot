...
export class DatabaseClient {
  ...
  updateCorrection(id: string, updates: Partial<CorrectionRow>): boolean {
    const sets: string[] = [];
    const values: unknown[] = [];
    ...
    if (updates.corrected_content !== undefined) {
      sets.push('corrected_content = ?');
      values.push(updates.corrected_content);
    }
    ...
    if (updates.target_section !== undefined) {
      sets.push('target_section = ?');
      values.push(updates.target_section);
    }
  ...
  getCorrection(id: string): CorrectionRow | undefined {
    return this.db.prepare(
      'SELECT * FROM corrections WHERE id = ? AND project_id = ?'
    ).get(id, this.projectId) as CorrectionRow | undefined;
  }

  deleteCorrection(id: string): boolean {
    const result = this.db.prepare(
      'DELETE FROM corrections WHERE id = ? AND project_id = ?'
  ...
  getCorrectionStats(): {
    ...
    const rows = this.db.prepare(
      'SELECT status, target, agent_id, COUNT(*) as count FROM corrections WHERE project_id = ? GROUP BY status, target, agent_id'
    ).all(this.projectId) as Array<{ status: string; target: string; agent_id: string | null; count: number }>;

    const byStatus: Record<string, number> = { pending: 0, approved: 0, rejected: 0, applied: 0, expired: 0 };
    const byTarget: Record<string, number> = { skill: 0, agent: 0, memory: 0, preference: 0 };
    ...
    for (const row of rows) {
      ...
      byTarget[row.target] = (byTarget[row.target] || 0) + row.count;
      if (row.agent_id) {
        byAgent[row.agent_id] = (byAgent[row.agent_id] || 0) + row.count;
      }
    }

    return {
      total,
      byStatus: byStatus as Record<CorrectionStatus, number>,
      byTarget: byTarget as Record<CorrectionTarget, number>,
      byAgent,
    };
  }

  expireCorrections(): number {
    const result = this.db.prepare(`
      UPDATE corrections SET status = 'expired', updated_at = ?
