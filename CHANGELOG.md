...
- **`templates/CLAUDE.template.md` trimmed to its routing-relevant minimum (TASK-127/W-5)**: the ladder harness's first live run (`copilot-control-tower` TASK-125, WP-44/WP-76) measured that adding the full framework scaffold to a session costs a ~24.2% marginal-token premium vs a bare harness, and that 98.3% of that premium is fixed `cache_creation` overhead paid once at session start, not costlier reasoning. A targeted ablation (real `claude -p` API calls, isolated `HOME`, same trivial no-tool prompt, `chars/4` heuristic cross-checked against measured `usage.cache_creation_input_tokens`) empirically located that overhead: `.claude/agents/*.md` (13 files) contribute only ~53 tokens/agent (~690 total) to a session's context — Claude Code loads only each subagent's frontmatter for Task-tool routing, not its full body, so agent definitions are **not** eagerly loaded in full and were **not** trimmed (their detail is read by the subagent itself, on delegation, and any cut there is pure capability risk for near-zero session-token benefit). `.claude/commands/*` and `.claude/skills/*` contribute similarly little (~9 and ~36 tokens/file respectively — name+description only, full bodies read on demand). `CLAUDE.md`, by contrast, **is** loaded in full every session and was the single largest lever: removed the template's "Agents" table (14 rows of name+domain, duplicated by the Task tool's own automatic subagent descriptions and by `~/.claude/copilot/README.md#your-team`, which is already referenced from the same file) and compressed the "Knowledge Copilot" domain-path table (5 rows) to a single pointer at the already-referenced consumption contract (detail read on first knowledge use, not needed for routing). Measured reduction (real API calls, `--model haiku`, isolated `HOME`, `--setting-sources project`): CLAUDE.md's own session contribution 1,081 → 680 tokens (-37.1%); full 13-agent+commands+skills+CLAUDE.md scaffold 2,124 → 1,846 tokens (-13.1%, n=1, some run-to-run cache-boundary noise expected). Raw file size 3,275 → 2,347 bytes (819 → 587 tokens, `chars/4` heuristic). No agent/command/skill file content changed; all six golden-set agent evals (`cc eval --agent {me,ta,qa,doc,sd,uxd} --json`) still 10/10 after the change. The ladder re-run to confirm this moves O-4 (`marginal_spend`) is owned by a concurrent lane, not verified here.

### Removed

- **`cpa`/`cs` agents cut (DEC-8, TASK-100)**: both showed 0 measured subagent invocations across the full framework_soul corpus, no measurement caveat, and no eval coverage. Removed `.claude/agents/{cpa,cs}.md`, their `manifest.json` entries and the `cs → cpa` routing edge, and all live-roster references (README, CLAUDE.md, docs, hook fallback lists, protocol/setup/update command roster fallbacks). Roster: 16 → 14 (13 framework + kc). `VERSION.json` `components.agents`: `count` 16 → 14, `version` 5.6.0 → 5.7.0. Reversible via git history. Historical docs (`docs/10-architecture/04-framework-restructure-2026-04.md`, explicitly marked Superseded) are left as-is — they describe a past roster state, not current.
