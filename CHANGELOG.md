...
Prepared as a candidate under codex-copilot PRD-9/TASK-16 (Claude Copilot `tc` PRD-4); the annotated, signed release tag remains pending the owner's signing key — see `docs/30-operations/09-release-preparation-5.15.0.md`. Framework `5.14.16` was never tagged or documented (no CHANGELOG entry existed for `5.14.11`–`5.14.16`); this release supersedes that number and absorbs its content as part of `5.15.0`, the first coherent, signable release since `5.14.10`.

- **One task-bound completion authority for delivery evidence (`tc` 1.4.0):** `tc.services.tasks.update_task` — the single completion-status writer — now gates any `status="completed"` transition on `metadata.requiresQa` through a new `tc.services.qa.check_task_qa`, exposed identically to the CLI (`tc task check-qa <id> [--json]`) and the Python API (`tc.api.check_task_qa`), and enforced under the same `BEGIN IMMEDIATE` write lock whether the caller owns its own connection or batches through `tc.api.transaction`. `requiresQa` stays an explicit, caller-set flag; an agent-based auto-default was evaluated and rejected because it broke three existing, protected tests pinning today's no-metadata behavior. Older, pre-evidence-schema work products are read under an explicit legacy policy and are never retroactively invalidated.
- **Versioned, pure evidence-packet validator (`tc` 1.4.0):** a new `tc.evidence` package (`schema.py`, `artifact_types.py`, `parse.py`, `validate.py`) gives the completion authority one place to parse and validate the `CRITERION/EXPECTED/OBSERVED/IDENTITY/BASELINE/ARTIFACT/UNTESTED/VERDICT` packet the QA playbook already documents — tolerant of legacy free-text work products, immune to a quoted or fenced example `VERDICT:` line, and closed against ever executing anything an artifact string names. `validate.py` imports nothing from `tc.db`, `tc.services`, `subprocess`, or `os`, checked by an import-purity fitness test.
- **One artifact-type registry for Claude, Codex and `tc` (`tc` 1.4.0):** `tc.evidence.artifact_types` is now the sole source of truth for accepted evidence types (`test-run`, `file-check`, `diff-check`, `adversarial-run`, `screenshot-check`, `a11y-check`, `design-fidelity-check`), closing a prior gap where `tc`'s own predicate silently rejected `adversarial-run` even though both the Claude hook and the QA playbook already treated it as sufficient on its own.
- **Claude's session hook now defers the QA verdict to the task authority (`agents` 5.7.0):** `.claude/hooks/subagent-stop.sh` no longer decides pass/fail from its own regex over the returned agent message — `parse_qa_verdict` and `has_artifact_marker` are removed. It now shells out to `tc task check-qa <task_id> --json` and clears only that exact task from the session's pending list, so one task's approval can never clear a different pending task. After three consecutive failures the session may still be advisory-unblocked, but the log and system message now say explicitly that the *session* is unblocked while the *task* remains unverified; only a passing `tc task check-qa` can ever mark a task verified.
- **Codex's gate distinguishes a missing capability from a real QA rejection (`tc` 1.4.0 / Codex parity):** `scripts/copilot-gate.sh` now preflights `tc task check-qa --help` and exits `3` with an explicit "capability unavailable" message when the installed `tc` predates this command, instead of reporting every QA-required task as failed. Exit codes are documented: `0` passed, `1` QA gate failed, `2` usage/environment error, `3` capability unavailable.
- **Optional-context instruction delivered to every shipped surface (`commands` 5.4.0, `agents` 5.7.0):** the `cc skill select` workflow — bounded, receipted, budget-aware optional-context loading — is now documented in a single canonical source (`.claude/agents/_shared/optional-context.md`) and embedded byte-identically into `ta.md`, `me.md`, `qa.md`, `protocol.md`, `templates/CLAUDE.template.md`, and the Codex-side `plugins/codex-copilot/skills/specialist-agents/references/shared-behaviors.md`, so a consuming project actually receives the same fallback and budget-overage behavior on every surface rather than only in the framework's own root `CLAUDE.md`.
- **Taste applicability and precedence clause in every taste-reading agent (`agents` 5.7.0):** `sd.md`, `ind.md`, `uxd.md`, `uids.md`, `cco.md`, `cw.md`, and `ta.md` each gain an identical clause restricting a personal taste rule to lenses matching the agent's own id and an `Applies:` line scoped to this project or `personal`, and stating explicitly that project constraints, repository instructions, and the Constitution outrank a personal rule on conflict — closing the cross-project leakage and personal-preference-outranks-project gaps named in the handoff.
- **`reflect.md` becomes a distributed project command (`commands` 5.4.0):** `VERSION.json`'s `components.commands.projectCommands` now lists `reflect.md`, so `/update-project`'s canonical reconciliation actually copies it into consuming projects; before this change the file existed in the framework source but neither the machine installer nor the project reconciler would ever distribute it. Its presence is now mandatory for every install — deleting or renaming it without a matching roster update will reject a project's entire source set.
- **`cc` remains at `2.12.15`, unchanged by this release:** its own undocumented `2.12.9`–`2.12.15` patch series predates this rollout and is a separate, preexisting gap not addressed here.
...
- **`cc connections --json` no longer reads a permanent false `needs-connect` for a tier's bootstrap credentials, and never renders a raw CLI error to a non-technical user (`cc` 2.2.1, WP-395 G-1/G-2):** a name that is structurally impossible for the shared store to ever hold (e.g. the Infisical machine-identity bootstrap pair, `INFISICAL_CLIENT_ID`/`_SECRET`) is now always presence-checked against the local OS keychain instead — a defensive backstop that holds even if a tier's overlay still (incorrectly) declares such a name `from: "any"`/`"store"`, mirroring cli-copilot's own `managed_store._NEVER_FROM_STORE` classification since `cc` cannot import cli-copilot code (verified live: this fix turns the owner's own machine from `needs-connect` to `ready` on that row, matching every other already-connected row). Separately, when the shared store's underlying call fails, `store.detail` is now always a plain-language, actor-routed sentence `cc` authors itself ("This Mac isn't connected to your organization's shared credential store yet. Your IT admin can set that up — nothing is wrong with your setup.") instead of a passthrough of the underlying command's raw stderr (which previously told a non-technical user to hand-edit a `.env` file that no longer even holds these credentials); the raw text is preserved only in a new, additive, machine-only `store.diagnostic` field, never rendered to a user. See `connections.schema.json`'s additive `store.diagnostic` property and `tools/cc/src/cc/commands/connections.py`'s module docstring.
- **Onboarding manifest write no longer hostage to materialize-content outcomes (`cc` 2.1.1):** `cc onboard --apply` now commits and keeps the layer-manifest write once topology is verified; a materialize `held` (a never-destroy guard protecting content, e.g. a symlink escaping the materialize root) or `blocked` (the fail-closed signature policy refusing unverified content) outcome is reported honestly in a new, additive `materialize` report field instead of rolling back a manifest whose topology was already proven correct. Only a genuinely unexpected materialize failure (an environment error or a raised exception) still fails the run and rolls back. Rollback confirmation now compares the manifest file's bytes directly rather than re-running the same materialize/policy gates against the restored manifest, which could previously self-report `rollback-failed` even when the file was byte-identical restored. The signature policy's `verify_git_item` now checks a layer's actually resolved/pinned `source.ref` when it is locally resolvable (falling back to its prior blind-HEAD check otherwise), so a foundation checkout that matches a signed release tag by content but sits on an unrelated local branch (`parentless-snapshot-match`) verifies correctly instead of being misreported as unverified.
