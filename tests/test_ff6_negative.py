...
def run_fitness_check():
    ...
    result = subprocess.run(
        ["bash", FITNESS_SCRIPT,
         ...
         "--commands-dir", os.path.join(REPO_ROOT, ".claude", "commands"),
         "--copilot-path", os.path.expanduser("~/.claude/copilot")],
        capture_output=True,
        text=True,
        cwd=REPO_ROOT,
    )
...
def test_positive_baseline_fitness_check_passes():
    ...
    assert rc == 0, f"Expected fitness check to PASS on clean repo but got rc={rc}:\n{output}"
    assert "FITNESS CHECK PASSED" in output, f"Expected PASS in output:\n{output}"
    assert "FF6" in output or "Stale Design" in output, f"FF6 section not found in output:\n{output}"
...
def test_negative_a_ff6_routing_stage_design_fails():
    ...
    bad_content = original + "\n\n<!-- QA TEST INJECTION -->\nBuild a feature: sd → design → ta → me → qa\n"
    write_file(CLAUDE_MD, bad_content)
    try:
        rc, output = run_fitness_check()
        assert rc != 0, f"Expected fitness check to FAIL with routing-stage 'design' but got rc=0:\n{output}"
        assert "[FAIL]" in output and "design" in output.lower(), \
...
def test_negative_b_ff1_agent_design_in_protocol_fails():
    ...
    try:
        rc, output = run_fitness_check()
        assert rc != 0, f"Expected fitness check to FAIL with @agent-design ref but got rc=0:\n{output}"
...
def test_negative_c_orphan_route_in_agent_fails():
    ...
    try:
        rc, output = run_fitness_check()
        assert rc != 0, f"Expected fitness check to FAIL with @agent-bogus but got rc=0:\n{output}"
        assert "bogus" in output.lower(), f"Expected 'bogus' to appear in FAIL output:\n{output}"
    finally:
...
def test_commit_7f6f081_exists_and_scope():
    ...
    files_changed = [line.strip().split()[0] for line in result.stdout.strip().splitlines() if "|" in line]
    expected_files = {"CLAUDE.md", "CHANGELOG.md", ".claude/fitness-check.sh"}
    actual_files = set(files_changed)
    unexpected = actual_files - expected_files
    assert not unexpected, f"Unexpected files in commit 7f6f081: {unexpected}\nAll changed: {actual_files}"
    for expected in expected_files:
        assert expected in actual_files, f"Expected {expected} in commit 7f6f081 but not found. Got: {actual_files}"
...
def test_version_json_matches_package_json():
    ...
    with open(os.path.join(REPO_ROOT, "package.json")) as f:
        pj = json.load(f)
    assert vj["framework"] == pj["version"], \
...
def test_changelog_has_5_3_0_section():
    ...
    assert "fitness-check" in content.lower() or "FF6" in content, \
        "CHANGELOG.md [5.3.0] section doesn't mention fitness-check/FF6"
    assert "roster" in content.lower() or "16-agent" in content.lower(), \
...
def test_design_agent_absent():
    ...
    assert not os.path.exists(design_path), f"design.md still present at {design_path} — must be removed"
...
def test_claude_md_no_stale_design_routing():
    ...
    routing_pattern = re.compile(r'(→\s*design\s*→|→\s*design\s*$|\bdesign\s*→)', re.MULTILINE)
    exempt_patterns = ["Design chain", "Atomic Design", "design tokens", "service design",
                       "visual design", "design chain"]
...
def test_claude_md_specification_workflow_correct():
    ...
    content = read_file(CLAUDE_MD)
    assert "(sd, design)" not in content, "CLAUDE.md still contains old '(sd, design)' in Specification Workflow"
    assert "(sd, ind, uxd, uids, cco, cw)" in content, \
