...
| Avoid reading >8 files directly | Delegate to framework agent | Hook: force-delegate (measured output and distinct-file budget; resets on `/compact`) |
| Keep responses short | Store details via `tc wp store` | Advisory |

**Mechanical enforcement:** The force-delegate rule, QA-gate rule, session-cap advisory, and safety primitives are enforced by hooks in `.claude/hooks/` — not just policy. Once a session's measured tool output passes 200KB (~50K tokens) or it has opened more than 10 distinct files with Read/Edit/Write, further main-session Bash/Read/Edit/Write calls are blocked until the work is delegated or the session is compacted (thresholds: `.claude/force-delegate-budget-baseline-v*.json`). After `@agent-me` completes, all main-session tools are gated until `@agent-qa` provides a pass verdict. **Safety primitives:** `/careful` (destructive-command block/warn via `security-rules.json`) and `/freeze` (edit-boundary lock via `.claude/hooks/state/.freeze`) — escape hatches: `COPILOT_CAREFUL=off`, `COPILOT_FREEZE=off`, `COPILOT_SAFETY=off`. See `.claude/hooks/README.md` for escape hatches and debug tools.
...
**Configurable at setup** (`/setup` machine-wide, overridable per project with `--project`):
- `cc config set output.verbosity <concise|standard|detailed>` — default `concise` if unset.
- `cc config set output.audience <plain|technical>` — default `plain` if unset.

Read via `eval "$(cc env)"` as `$CC_OUTPUT_VERBOSITY` / `$CC_OUTPUT_AUDIENCE` — already step 2 of every agent's Workflow, so no extra hydration step is needed.
