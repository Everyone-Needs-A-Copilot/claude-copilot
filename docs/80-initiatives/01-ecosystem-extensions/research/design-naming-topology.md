...
products:                             # which families this org has enabled + pinned floors
  ...
  knowledge:{ enabled: true,  foundation: "^2.3.0", topology: separate }   # per-product topology (§5); separate = default
...
layers:
  ...
  # --- knowledge product (separate-repo topology → own dept repo; the default) ---
  ...
  - { id: dept-know,    product: knowledge, role: department, unit: finance, rank: 20,
      source: { repo: git@github-work:acme-corp/copilot-knowledge-dept-finance.git }, auth: ssh-work }  # own team-scoped repo
...
> **Updated per owner decision (2026-07-06):** department content is confidential business data (financial figures, forecasts, proprietary methods and processes) → **separate-repo is the default across all products**, including `knowledge`; `subfolder` is a non-confidential opt-in, not the typical case.
...
| **knowledge** | **Option A — separate repo** `copilot-knowledge-dept-<slug>` | Department knowledge is confidential business data by default (financials, forecasts, proprietary process); the repo is GitHub's only read boundary. `subfolder` (`copilot-knowledge-org//departments/<slug>`) remains available as an explicit opt-in when a department's content is affirmatively non-confidential — it kills org↔dept version skew (one SHA) and makes dept→org promotion a `git mv`. |
...
| Dept topology | **separate repo per department, all products (default — the repo is GitHub's only read boundary; confidential dept content)**; `subfolder` is a narrow opt-in for affirmatively non-confidential content only (§5, updated per owner decision 2026-07-06) |
