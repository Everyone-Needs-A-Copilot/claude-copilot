...
**Threshold, re-baselined as policy v1.1.0.** 200,000 measured bytes (~50,000 tokens). That is the v1.0.0 basis, 5% of the context window, applied to the 1M-token window sessions now run with. Calibrated against the 38 retained main-session transcripts, measured the way the hook now measures: 17 exceeded 40,000 bytes, so the old limit tripped on ordinary work once honestly measured; 2 exceeded 200,000 (311,691 and 363,023), which is the sprawl the rule exists for; none exceeded 400,000. The distinct-files meter rises from five to ten on the same data: five was exceeded by 8 of the 38 sessions (21%), mostly ordinary work, while ten is exceeded by exactly 2 (17 and 21 files), the same sprawl rate the byte budget targets. The pre-call floor exemption is gone, because it only existed to soften pre-call guesses.
