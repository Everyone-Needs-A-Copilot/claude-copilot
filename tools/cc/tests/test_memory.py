...
class TestEntryStore:
    def test_store_creates_uuid_md_file(self, memory_root):
        result = store_entry(
            entry_type="decision",
            content="Use SQLite for search cache.",
            scope="project",
        )
    ...
    def test_store_writes_correct_frontmatter(self, memory_root):
        result = store_entry(
            entry_type="lesson", content="Always test.", tags=["test"], scope="project"
        )
        path = Path(result["path"])
    ...
    def test_get_by_full_uuid(self, memory_root):
        stored = store_entry(
            entry_type="reference", content="Full UUID retrieval.", scope="project"
        )
    ...
    def test_get_by_prefix_uuid(self, memory_root):
        stored = store_entry(
            entry_type="context", content="Prefix match test.", scope="project"
        )
        prefix = stored["id"][:8]
    ...
    def test_get_ambiguous_prefix_raises(self, memory_root):
        ...
        for suffix in (
            "aaaa-0000-0000-0000-000000000001",
            "aaaa-0000-0000-0000-000000000002",
        ):
    ...
    def test_list_filter_by_tag(self, memory_root):
        store_entry(
            entry_type="context", content="tagged", tags=["auth"], scope="project"
        )
    ...
    def test_search_finds_content(self, memory_root):
        ...
        store_entry(
            entry_type="decision", content="Choose SQLite FTS5.", scope="project"
        )
    ...
    def test_search_case_insensitive(self, memory_root):
        store_entry(
            entry_type="context", content="PostgreSQL is great.", scope="project"
        )
    ...
    def test_search_finds_in_frontmatter_tags(self, memory_root):
        store_entry(
            entry_type="decision", content="body", tags=["uniquetag"], scope="project"
        )
...
class TestMemoryCLI:
    ...
    def test_store_exits_zero(self, cli_runner, cli_app, monkeypatch, tmp_path):
        ...
        result = cli_runner.invoke(
            cli_app, ["memory", "store", "--type", "lesson", "Test lesson content"]
        )
    ...
    def test_store_json_output(self, cli_runner, cli_app, monkeypatch, tmp_path):
        ...
        result = cli_runner.invoke(
            cli_app,
            ["memory", "store", "--type", "decision", "--json", "Decision content"],
        )
        assert result.exit_code == 0
    ...
    def test_list_empty_shows_no_entries(
    ...
    def test_store_then_list(self, cli_runner, cli_app, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(es, "_git_root", lambda: tmp_path)
        cli_runner.invoke(
            cli_app, ["memory", "store", "--type", "decision", "My decision"]
        )
        result = cli_runner.invoke(cli_app, ["memory", "list"])
        assert result.exit_code == 0
    ...
    def test_get_by_id(self, cli_runner, cli_app, monkeypatch, tmp_path):
        ...
        store_result = cli_runner.invoke(
            cli_app, ["memory", "store", "--type", "lesson", "--json", "Lesson body"]
        )
    ...
    def test_delete_with_yes(self, cli_runner, cli_app, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(es, "_git_root", lambda: tmp_path)
        store_result = cli_runner.invoke(
            cli_app, ["memory", "store", "--type", "context", "--json", "To delete"]
        )
        data = json.loads(store_result.output)
        entry_id = data["id"]
    ...
    def test_search_finds_content(self, cli_runner, cli_app, monkeypatch, tmp_path):
        ...
        cli_runner.invoke(
            cli_app,
            ["memory", "store", "--type", "context", "Unique searchable phrase xyzzy"],
        )
    ...
    def test_index_rebuild(self, cli_runner, cli_app, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(es, "_git_root", lambda: tmp_path)
        cli_runner.invoke(
            cli_app, ["memory", "store", "--type", "lesson", "index test"]
        )
    ...
    def test_index_status_in_sync(self, cli_runner, cli_app, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(es, "_git_root", lambda: tmp_path)
        cli_runner.invoke(
            cli_app, ["memory", "store", "--type", "context", "status test"]
        )
    ...
    def test_index_status_out_of_sync_exits_3(
        self, cli_runner, cli_app, monkeypatch, tmp_path
    ):
    ...
    def test_index_no_flags_exits_nonzero(
        self, cli_runner, cli_app, monkeypatch, tmp_path
    ):
        import cc.core.entry_store as es
