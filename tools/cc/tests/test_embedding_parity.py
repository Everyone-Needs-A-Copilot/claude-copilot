...
class TestOffPathParity:
    ...
    CORPUS = [
        (
            "lesson",
            ["python", "testing"],
            "pytest is the best python testing framework",
        ),
        ("context", ["deployment", "ci"], "github actions runs our ci pipeline"),
        (
            "decision",
            ["architecture"],
            "we chose sqlite for local storage due to simplicity",
        ),
        (
            "lesson",
            ["python"],
            "python type hints improve code clarity and refactoring",
        ),
    ]
    ...
    def _seed_and_rebuild_fts(self, memory_root):
        ...
        for entry_type, tags, content in self.CORPUS:
            store_entry(
                entry_type=entry_type, content=content, tags=tags, scope="project"
            )
    ...
    def test_module_search_with_none_matches_fts5_directly(
        self, memory_root, monkeypatch
    ):
    ...
    def test_result_dict_shape_identical(self, memory_root):
        ...
        store_entry(
            entry_type="lesson", content="shape test content zork", scope="project"
        )
        fts = FTS5Backend()
        fts.rebuild(memory_root)
        set_default_backend(FTS5Backend())
...
class TestNoImportFitness:
    ...
    def test_fts5_search_does_not_import_sentence_transformers(self, memory_root):
        ...
        store_entry(
            entry_type="lesson", content="no import fitness test delta", scope="project"
        )
        set_default_backend(FTS5Backend())
...
class TestGracefulDegradationFactory:
        ...
        with patch.dict(sys.modules, {"sentence_transformers": None}):
            with caplog.at_level(logging.WARNING, logger="cc.core.backend_factory"):
                backend = resolve_backend(
                    _model_override="all-MiniLM-L6-v2", _force_reload=True
                )
    ...
    def test_degraded_backend_still_searches(self, memory_root, caplog):
        ...
        store_entry(
            entry_type="lesson",
            content="degradation search works gamma",
            scope="project",
        )
        ...
        with patch.dict(sys.modules, {"sentence_transformers": None}):
            with caplog.at_level(logging.WARNING):
                backend = resolve_backend(
                    _model_override="some-model", _force_reload=True
                )
        ...
        assert any(
            "degradation search works gamma" in r.get("content", "") for r in results
        )
    ...
    def test_no_crash_on_embedding_failure(self, memory_root, caplog):
        ...
        store_entry(
            entry_type="lesson", content="crash safety test epsilon", scope="project"
        )
        ...
        assert isinstance(
            results, list
        ), "search() must not raise even when encode fails"
...
class TestDefaultOff:
    ...
    def test_no_embeddings_table_created_when_off(self, memory_root, monkeypatch):
        ...
        store_entry(
            entry_type="lesson", content="off path no table test", scope="project"
        )
        rebuild_index(memory_root)
