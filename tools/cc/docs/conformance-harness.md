...
| `tier.effectiveness.extension_resolution_wired_beyond_prose` (E-6, S2) | `cc extensions resolve` is invoked by an executable hook/script somewhere in the framework's legitimate executable surfaces, not merely described in `.md` prose; the check emits one framework-wide verdict and does not require cargo-cult duplicate invocations in every sibling directory | a fixture `.sh` file with only a commented-out invocation |

`tests/conformance/test_layer1_effectiveness.py` carries the PASS-shape and FAIL-shape fixture for each, plus one `@pytest.mark.machine` test per check. E-6 now passes because `.claude/hooks/pretool-check.sh` is the legitimate executable consumer; `plugins/` and `scripts/` are scanned as part of the same capability surface and are not required to duplicate the hook. Negative fixtures continue to prove the detector fails when the invocation is absent or only commented out; `expected_today` is set per result to match observed evidence, never weakened to make a number look better.
