...
def skill_evaluate(
    files: Optional[str] = typer.Option(
        None,
        "--files",
        help="Comma-separated file paths to evaluate against skill triggers.",
    ),
    text: Optional[str] = typer.Option(
        None,
        "--text",
        help="Text snippet to evaluate against skill triggers.",
    ),
    threshold: float = typer.Option(
        0.5,
        "--threshold",
        help="Minimum confidence threshold (0.0–1.0). Not yet implemented.",
    ),
    scope: Optional[str] = typer.Option(
        "all",
        "--scope",
    ...
    """Evaluate which skills are relevant for given files and/or text.

    Currently performs keyword-based matching. Semantic scoring (Stream-A2)
    will be layered in once embeddings are available.
    """
    from cc.core.skill_store import search_skills

    if not files and not text:
        err_console.print("[red]Error:[/red] Provide --files and/or --text.")
        raise typer.Exit(1)

    query_parts: list[str] = []
    if files:
        # Include file extensions and basenames as search tokens
        for f in files.split(","):
            p = Path(f.strip())
            query_parts.append(p.name)
            if p.suffix:
                query_parts.append(p.suffix.lstrip("."))
    if text:
    ...
    if not results:
        console.print("[dim]No relevant skills found.[/dim]")
        return
    ...
    for skill in results:
        table.add_row(skill.name, skill.description or "", ", ".join(skill.tags))
