"""SQLite FTS5 index for memory entries (local cache only, gitignored)."""
...
def rebuild_index(memory_root: Path) -> dict[str, int]:
    """
    Drop and rebuild the FTS index from all .md files in entries/.

    Returns {"indexed": <count>, "errors": <count>}.
    """
    e_dir = entries_dir(memory_root)
    db_path = _db_path(memory_root)
...
def index_status(memory_root: Path) -> dict[str, Any]:
    """
...
def search_index(query: str, memory_root: Path) -> list[dict[str, Any]]:
    """
    FTS5 search against the SQLite index.
...
def index_entry(entry_id: str, entry_type: str, tags: list[str], content: str, memory_root: Path) -> None:
    """Insert or replace a single entry in the FTS index."""
    db_path = _db_path(memory_root)
    conn = _open_db(db_path)
    try:
        # Remove existing row for this id first
...
def remove_from_index(entry_id: str, memory_root: Path) -> None:
