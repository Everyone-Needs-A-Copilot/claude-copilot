...
def test_update_task_completion_logs_action(db_path):
    ...
    conn = get_db(db_path)
    log = conn.execute(
        "SELECT * FROM agent_log WHERE task_id = ? AND action = 'completed'",
        (task["id"],),
    ).fetchone()
    conn.close()
...
def test_update_prd_success(db_path):
    ...
    updated = update_prd(
        prd_id=prd["id"], title="New Title", status="completed", db_path=db_path
    )
...
def test_search_wps_returns_results(db_path):
    ...
    store_wp(
        task_id=task["id"],
        type_="analysis",
        title="Unique keyword: xyzzy123",
        content="xyzzy123 content",
        db_path=db_path,
    )
...
def test_handoff_task_logs_action(db_path):
    ...
    conn = get_db(db_path)
    log = conn.execute(
        "SELECT * FROM agent_log WHERE task_id = ? AND action = 'handoff'",
        (task["id"],),
    ).fetchone()
    conn.close()
    assert log is not None
...
def test_list_log_filter_by_task(db_path):
    ...
    handoff_task(
        task_id=t1["id"], from_agent="me", to_agent="qa", context="a", db_path=db_path
    )
    handoff_task(
        task_id=t2["id"], from_agent="me", to_agent="qa", context="b", db_path=db_path
    )
